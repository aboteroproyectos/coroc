...
void main() {
  testWidgets('la sesión recordada se restaura y abre el tablero; cada sección carga sin errores', (tester) async {
    ...
    for (final route in ['/dashboard', '/today', '/clients', '/clients/${app.api.id('client')}', '/inbox', '/messages', '/messages/templates', '/reports', '/settings', '/help', '/clients/new', '/clients/${app.api.id('client')}/loans/new']) {
      await app.go(route);
