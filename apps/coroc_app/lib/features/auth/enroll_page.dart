...
class _EnrollPageState extends ConsumerState<EnrollPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return AuthFrame(title: l.enrollTitle, subtitle: l.enrollSubtitle, children: [
      FutureBuilder<MfaEnrollment>(
        future: _enrollment,
        builder: (context, snap) {
          if (snap.hasError) {
            return ErrorState(message: errorText(context, snap.error!), onRetry: () => setState(() {
                  _enrollment = ref.read(authProvider.notifier).startEnrollment();
                }));
          }
          ...
          return Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
            ...
            SelectableText(_group(e.secret), textAlign: TextAlign.center, style: Theme.of(context).textTheme.titleSmall?.copyWith(fontFamily: 'Inter', letterSpacing: 1.5)),
            TextButton.icon(
              onPressed: () {
