    ...
    final ok = await showDialog<bool>(
      ...
      builder: (c) => AlertDialog(
          ...
          child: Column(
            ...
            children: [
              TextField(
                controller: holder,
                autofocus: true,
                decoration: corocInput(context, label: l.receivingHolder, helper: l.receivingHolderHelp),
              ),
              const SizedBox(height: CorocSpace.md),
              TextField(
                controller: entity,
                decoration: corocInput(context, label: l.receivingInstitution),
              ),
              const SizedBox(height: CorocSpace.md),
              TextField(
                controller: last4,
                ...
                decoration: corocInput(context, label: l.receivingLast4),
              ),
            ],
          ),
        ),
        actions: [
          ...
          FilledButton(onPressed: () => Navigator.pop(c, holder.text.trim().isNotEmpty), child: Text(l.actionSave)),
        ],
      ),
    );
    if (ok == true) {
      try {
        await ref
            .read(apiProvider)
            .addReceivingAccount(holderName: holder.text.trim(), institution: entity.text.trim().isEmpty ? null : entity.text.trim(), last4: last4.text.length == 4 ? last4.text : null);
        ...
        builder: (c) {
          ...
          Widget number(String label, int value, List<int> options, String key) => Padding(
            ...
            child: DropdownMenu<int>(
              ...
              dropdownMenuEntries: [
                for (final o in {...options, value}.toList()..sort()) DropdownMenuEntry(value: o, label: '$o'),
              ],
            ),
          );
          return Column(
            ...
            children: [
              SegmentedButton<String>(
                segments: [
                  ButtonSegment(value: 'auto_with_audit', label: Text(l.autoModeAudit)),
                  ButtonSegment(value: 'prior_approval', label: Text(l.autoModePrior)),
                ],
                selected: {mode},
...
class WhatsAppSection extends ConsumerWidget {
  ...
  Future<void> _mode(BuildContext context, WidgetRef ref, WhatsAppAccount a, String mode) async {
    ...
    if (mode == 'cloud_api') {
      checklist = await showDialog<WhatsAppChecklist>(
        context: context,
        builder: (_) => _ChecklistDialog(initial: a.checklist),
      );
  ...
  Future<void> _edit(BuildContext context, WidgetRef ref, WhatsAppAccount a) async {
    ...
    final ok = await showDialog<bool>(
      ...
      builder: (c) => AlertDialog(
        ...
        content: SizedBox(
          ...
          child: Column(
            ...
            children: [
              TextField(
                controller: number,
                decoration: corocInput(context, label: l.whatsappDisplay),
              ),
              const SizedBox(height: CorocSpace.md),
              TextField(
                controller: id,
                keyboardType: TextInputType.number,
                decoration: corocInput(context, label: l.whatsappPhoneId),
              ),
              const SizedBox(height: CorocSpace.md),
              TextField(
                controller: waba,
                keyboardType: TextInputType.number,
                decoration: corocInput(context, label: l.whatsappWabaId, helper: l.whatsappWabaIdHelp),
              ),
              const SizedBox(height: CorocSpace.md),
              TextField(
                controller: token,
                obscureText: true,
                decoration: corocInput(context, label: l.whatsappToken, helper: l.whatsappTokenHelp),
              ),
            ],
          ),
        ),
        actions: [
          TextButton(onPressed: () => Navigator.pop(c, false), child: Text(l.actionCancel)),
          FilledButton(onPressed: () => Navigator.pop(c, id.text.trim().isNotEmpty && token.text.trim().length >= 20), child: Text(l.actionSave)),
        ],
      ),
    );
    if (ok == true) {
