...
/// Número de WhatsApp Business que recibe comprobantes (§12.1). El token se guarda cifrado y nunca vuelve a mostrarse.
class WhatsAppSection extends ConsumerWidget {
  const WhatsAppSection({super.key, required this.canEdit});
  final bool canEdit;
  ...
  Future<void> _edit(BuildContext context, WidgetRef ref, WhatsAppAccount a) async {
    ...
    if (ok == true) {
      try {
        await ref.read(apiProvider).saveWhatsAppAccount(phoneNumberId: id.text.trim(), accessToken: token.text.trim(), displayNumber: number.text.trim().isEmpty ? null : number.text.trim());
        ref.invalidate(whatsAppAccountProvider);
      } catch (e) {
        if (context.mounted) _toast(context, errorText(context, e));
      }
    }
    for (final c in [id, token, number]) {
      c.dispose();
    }
  }
    ...
    return SectionCard(
      ...
      child: AsyncBody<WhatsAppAccount>(
        ...
        builder: (a) => Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
          ...
          const SizedBox(height: 8),
          if (a.configured) StatusDot(label: l.whatsappConnected(a.displayNumber ?? a.phoneNumberId ?? ''), tone: StatusTone.ok) else StatusDot(label: l.whatsappNotConnected, tone: StatusTone.neutral),
