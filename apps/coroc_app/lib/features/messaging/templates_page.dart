...
class _TemplatesPageState extends ConsumerState<TemplatesPage> {
  ...
  Widget build(BuildContext context) {
    ...
    final lang = _lang ?? switch (context.lang) { 'pt' => 'pt-BR', 'en' => 'en', _ => 'es' };
    ...
    return PageScaffold(maxWidth: 1000, children: [
      ...
      AsyncBody<TemplatesView>(
        ...
        builder: (v) => Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
...
class _TemplateEditorState extends ConsumerState<_TemplateEditor> {
  ...
  void _insert(String name) {
    ...
    _body.value = TextEditingValue(text: text.replaceRange(start, end, token), selection: TextSelection.collapsed(offset: start + token.length));
    _changed();
  }
  ...
  Future<void> _save({bool restore = false}) async {
    ...
    final l = context.l10n;
    try {
      await ref.read(apiProvider).saveTemplate(
  ...
  Widget build(BuildContext context) {
    ...
    final editor = Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
      TextField(
        controller: _body,
      ...
      if (widget.template.event != 'manual')
        TextField(controller: _subject, enabled: widget.canEdit, onChanged: (_) => _changed(), decoration: corocInput(context, label: l.templatesSubject)),
      const SizedBox(height: 8),
      Text(l.templatesVariables, style: t.labelMedium),
      const SizedBox(height: 4),
      Wrap(spacing: 6, runSpacing: 6, children: [
        for (final v in templateVariables) ActionChip(label: Text('{{$v}}', style: t.bodySmall), onPressed: widget.canEdit ? () => _insert(v) : null),
      ]),
      if (widget.canEdit)
        ExpansionTile(
          ...
          children: [
            TextField(controller: _meta, onChanged: (_) => setState(() => _dirty = true), decoration: corocInput(context, label: l.templatesMetaName)),
            const SizedBox(height: 8),
            TextField(controller: _metaLang, onChanged: (_) => setState(() => _dirty = true), decoration: corocInput(context, label: l.templatesMetaLang)),
            const SizedBox(height: 8),
          ],
        ),
    ]);
          ...
          for (final i in p.issues)
            Padding(
              ...
              child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                Expanded(child: Text(issueText(l, i), style: t.bodySmall?.copyWith(color: Theme.of(context).colorScheme.error))),
              ]),
            ),
        ],
      ]),
    ...
    return SectionCard(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
        if (wide)
          Row(crossAxisAlignment: CrossAxisAlignment.start, children: [Expanded(child: editor), const SizedBox(width: CorocSpace.md), Expanded(child: preview)])
        else ...[editor, const SizedBox(height: CorocSpace.md), preview],
        if (widget.canEdit) ...[
          const SizedBox(height: CorocSpace.md),
